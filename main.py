...
def fire_bullets(player: Player, bullets: list, kills: int = 0) -> None:
    by = player.rect.top
    ...
    if bt == "triple":
        for ox in (-14, -2, 10):
            bullets.append(Bullet(cx + ox - w // 2, by, w, h, 12, style))
    elif bt == "double":
        for ox in (-10, -2):
            bullets.append(Bullet(cx + ox - w // 2, by, w, h, 12, style))
    else:
        bullets.append(Bullet(cx - w // 2, by, w, h, 12, style))
...
def _boss_fire(boss: Boss, enemy_bullets: list, players: list[Player],
    ...
    spread_count = min(3 + level, 7)
    spread_range = 0.3 + level * 0.05
    for i in range(spread_count):
        offset = (i - spread_count // 2) * (spread_range * 2 / max(1, spread_count - 1))
        angle = base_angle + offset
        spd = settings.BOSS_BULLET_SPEED + level * 0.2
        vx = math.cos(angle) * spd
        vy = math.sin(angle) * spd
        enemy_bullets.append(EnemyBullet(
            boss.rect.centerx, boss.rect.bottom,
            vx=vx, vy=vy, is_boss=True,
        ))
        ...
        while running:
            ...
            if touch is not None:
                if lives_list[0] > 0 and fire_cooldowns[0] <= 0:
                    fire_bullets(players[0], bullets, kills)
            ...
            if boss_warning_active:
                ...
                boss.update()
                if boss.should_fire(settings.BOSS_FIRE_INTERVAL):
            ...
            for pu in powerups[:]:
                for i, p in enumerate(players):
                    ...
                    if pygame.sprite.collide_rect(p, pu):
                        if pu.ptype == "bullet":
                            levels = [
                                ("normal", "normal"), ("double", "normal"), ("triple", "normal"),
                                ("triple", "laser"), ("triple", "plasma"), ("triple", "electric"),
                            ]
                            cur = (p.bullet_type, p.bullet_style)
                            idx = next((j for j, L in enumerate(levels) if L == cur), 0)
                            nxt = levels[3] if idx == 5 else levels[idx + 1]
