...
class Bullet:
    ...
    def __init__(self, x: float, y: float, width: int = 4, height: int = 12,
                 ...
                 vx: float = 0.0, vy: float | None = None):
        self.rect = pygame.Rect(x, y, width, height)
        self.speed = speed
        self.style = style
        self.vx = float(vx)
    ...
    def is_off_screen(self, screen_height: int) -> bool:
        return self.rect.bottom < -20 or self.rect.top > screen_height + 20 or \
               self.rect.right < -20 or self.rect.left > 500
...
class Boss:
    """Boss 敌机 - 每关不同外观/大小/行为"""

    BOSS_CONFIGS = {
        1: {"w": 110, "h": 75,  "hp": 30,  "spd": 2.0, "pts": 25},
        2: {"w": 120, "h": 80,  "hp": 45,  "spd": 2.5, "pts": 35},
        3: {"w": 130, "h": 90,  "hp": 65,  "spd": 2.8, "pts": 50},
        4: {"w": 145, "h": 95,  "hp": 90,  "spd": 3.0, "pts": 70},
        5: {"w": 160, "h": 105, "hp": 120, "spd": 3.2, "pts": 100},
        6: {"w": 170, "h": 110, "hp": 160, "spd": 3.5, "pts": 140},
        7: {"w": 180, "h": 115, "hp": 200, "spd": 3.8, "pts": 180},
    }

    def __init__(self, screen_width: int, screen_height: int, level: int):
        cfg_key = min(level, 7)
        cfg = self.BOSS_CONFIGS.get(cfg_key, self.BOSS_CONFIGS[7])
        extra = max(0, level - 7)
        self.width = cfg["w"] + extra * 5
        self.height = cfg["h"] + extra * 3
        self.screen_width = screen_width
        self.screen_height = screen_height
        self.level = level
        self.max_hp = cfg["hp"] + extra * 30
        self.hp = self.max_hp
        x = (screen_width - self.width) // 2
        self.rect = pygame.Rect(x, -self.height, self.width, self.height)
        self.target_y = 50
        self.entering = True
        self.speed = cfg["spd"] + extra * 0.2
        self.direction = 1
        self.points = cfg["pts"] + extra * 20
        self.fire_timer = 0
    ...
    def update(self) -> None:
        ...
        if self.level >= 4:
            phase_len = 180
            phase = (self.pattern_timer // phase_len) % 3
            if phase == 0:
                sway = math.sin(self.pattern_timer / 40.0) * 2.5
                self.rect.x += int(self.speed * self.direction + sway)
            elif phase == 1:
                self.rect.x += int(self.speed * 1.8 * self.direction)
            else:
                sway = math.sin(self.pattern_timer / 25.0) * 3
                self.rect.x += int(sway)
                vert = math.sin(self.pattern_timer / 50.0) * 1.5
                self.rect.y = self.target_y + int(vert * 10)
        else:
        ...
        if self.rect.right >= self.screen_width or self.rect.left <= 0:
            self.direction *= -1
        self.rect.x = max(0, min(self.screen_width - self.width, self.rect.x))
        self.rect.y = max(20, min(self.screen_height // 3, self.rect.y))
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        sprite = pa.boss_sprite(self.level, frame)
        scaled = pygame.transform.scale(sprite, (self.rect.width, self.rect.height))
        surface.blit(scaled, self.rect)
